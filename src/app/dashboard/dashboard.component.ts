...
export class DashboardComponent implements OnInit {
  ...
  //   this.hBatchFrom = moment(new Date()).format('MM/DD/YYYY');
  //   this.hBatchTo = moment(new Date()).format('MM/DD/YYYY');
  //   this.fromDate1 = moment(this.hBatchFrom).format('YYYY-MM-DD') + "  00:00:00";
  //   this.toDate1 = moment(this.hBatchTo).format('YYYY-MM-DD') + " 23:59:59";
  //   this.dropIn = [];
  //   this.dItem = [];
  //   this.dQty = [];
  //   this.stockQtyData = [];
  ...
  //   this.dashboardService.getStockQty().subscribe(res => {
  //     //this.stockQtyData = new Array<>();

  //     if (res.responseCode== 1) {
  //       this.stockQtyData.length = 0;
  //       this.stockQtyData = res.responseData;
  //       console.log("responseData>>>>>>>",this.stockQtyData)

  //       this.stockQtyData.forEach(element => {
  //         this.newStockData = element;
  //         console.log("responseData>>>>>>>",this.newStockData)
  //         this.stQtyArray.push(element['stockQty']);
  //         this.phQtyArray.push(element['phyQty']);
  //         this.vaQtyArray.push(element['variance']);
  //         this.stLocation.push(element['location']);

  //       });
  //       console.log("ressssssss",this.stockQtyData[0].stockQty);
  //       this.StockChart(this.stQtyArray,this.phQtyArray,this.vaQtyArray,this.stLocation)
  ...
  getStockQty() {
    ...
    let me = this;
    var ws = new WebSocket(AppParams.WEB_SOCKET_PATH + "getStockDetails");
    ...
    ws.onmessage = function (event) {
      ...
      if (mapJson['responseCode'] == 1) {
        ...
        data.forEach(element => {
          ...
          // console.log("newStockData",me.newStockData);
          me.stQtyArray.push(element['stockAmount']);
          me.phQtyArray.push(element['phyQty']);

          if (element['variance'] > 0) {
            me.vaQtyArray.push(element['variance']);
          } else {
            me.vaQtyArray.push((element['variance']) * (-1));
          }

          // me.stLocation.push(element['location']);

        });

        me.StockChart(me.stQtyArray, me.phQtyArray, me.vaQtyArray, me.stLocation);
        // me.allInOneChart();
        // me.productionChart(me.productArrayForIssue, me.productArrayForIssueData1, me.productArrayForIssueData2);

        me.loadingMask = false;
      }
    }
  ...
  getTrans() {
    ...
    this.loadingMask = true;
    let me = this;
    var ws = new WebSocket(AppParams.WEB_SOCKET_PATH + "getTrans");
    ws.onopen = function (event) {
    ...
    ws.onmessage = function (event) {
      ...
      if (mapJson['responseCode'] == 1) {
        ...
        me.getTransArray = data;
        // console.log("getTransArray >>>>>>>>> ",  me.getTransArray);
  ...
  //   this.dashboardService.getTrans().subscribe(res =>{

  //     if(res.responseCode==1){
  //       this.getTransArray;
  //       this.getTransArray = res.responseData;
  //       console.log("getTransArray >>>>>>>>> ",  res.responseData);
  //       console.log("getTransArray >>>>>>>>> ",  this.getTransArray);
  //     }else{
  //       console.log("getTransArray >>>>>>>>> ",  this.getTransArray);
  //     }
  //   }, error =>{
